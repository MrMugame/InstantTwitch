...
<main>
    <div class="flex justify-between w-full border-b-[1px] border-lightborder dark:bg-foreground bg-lightforeground relative">
        <div class="flex flex-row p-1">
            <input bind:value={input} on:input={res => filterStreams(res)} type="text" placeholder="Search" class="dark:bg-background bg-lightbackground h-6 w-48 rounded-md pl-2 dark:text-strongtext text-lightstrongtext mr-2 text-xs">
            <svg on:click={_ => promise = refreshStreams()} xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 dark:fill-lighttext dark:hover:fill-strongtext fill-lightstrongtext hover:fill-lightlighttext transition-colors" viewBox="0 0 20 20" fill="currentColor">
