...
<main>
    <div class="flex justify-between w-full border-b-[1px] border-lightborder bg-foreground relative">
        <div class="flex flex-row p-1">
            <input bind:value={input} on:input={res => filterStreams(res)} type="text" placeholder="Search" class="bg-background h-6 rounded-md pl-2 text-strongtext mr-2">
        ...
        {:catch}
            <button class="bg-twitch rounded-md px-3 border-[1px] border-lightborder text-strongtext font-semibold m-1" on:click={_ => {chrome.runtime.sendMessage({ data: "OAUTH" })}}>
                Login
            </button>
        {/await}
    </div>
