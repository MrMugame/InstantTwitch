...
<div on:click={openTab} class="w-full h-20 flex p-3 border-b-[1px] dark:text-lighttext text-lightlighttext border-lightborder group cursor-pointer overflow-hidden transition-colors">
    <div class="relative h-full bg-twitch">
        <img class="h-full aspect-video max-w-none group-hover:-translate-x-1 group-hover:-translate-y-1 transition-all" src={parseThumbnailUrl(stream.thumbnail_url)} alt="thumbnail">
