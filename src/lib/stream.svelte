...
<div on:click={openTab} class="w-full h-20 flex text-strongtext p-3 border-b-[1px] border-lightborder hover:bg-twitch cursor-pointer overflow-hidden transition-colors">
    ...
    <div class="ml-3">
        ...
        <h2 class="font-roboto text-lighttext text-xs">{stream.game_name} - {stream.viewer_count} Viewer</h2>
        <h2 class="font-roboto text-lighttext w-[300px] overflow-ellipsis whitespace-nowrap overflow-hidden inline-block text-xs">{stream.title}</h2>
    </div>
</div>