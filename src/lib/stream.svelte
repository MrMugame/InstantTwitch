...
<div on:click={openTab} class="w-full h-20 flex  p-3 border-b-[1px] border-lightborder hover:bg-twitch cursor-pointer overflow-hidden transition-colors">
    ...
    <div class="ml-3">
        <h1 class="text-xl dark:text-strongtext text-lightstrongtext bg-opacity-5leading-5 font-roboto font-bold mb-1">{stream.user_name}</h1>
        <h2 class="font-roboto dark:text-lighttext text-lightlighttext text-xs">{stream.game_name} - {stream.viewer_count} Viewer</h2>
        <h2 class="font-roboto dark:text-lighttext text-lightlighttext w-[300px] overflow-ellipsis whitespace-nowrap overflow-hidden inline-block text-xs">{stream.title}</h2>
    </div>
</div>